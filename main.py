...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        # Visualize xy grid using vis_grid
        if cam_idx == 0 and file_prefix == '':
            # pass
            xy_vis = vis_grid(xy_grid, image_size)
            plt.imsave("./images/grid_vis.png", xy_vis)

        # Visualize rays using vis_rays
        if cam_idx == 0 and file_prefix == '':
            # pass
            rays_vis = vis_rays(ray_bundle, image_size)
            plt.imsave("./images/rays_vis.png", rays_vis)
        
        #  Implement point sampling along rays in sampler.py
        ray_bundle = model.sampler(ray_bundle)

        #  Use render_points to visualize sample points as point cloud
        if cam_idx == 0 and file_prefix == '':
            # pass
            points = ray_bundle.sample_points.reshape(1, -1, 3)
            render_points("./images/points_vis.png", points)
        ...
        if cam_idx == 2 and file_prefix == '':
            # pass
            img = np.array(out["depth"].view(image_size[1], image_size[0]).detach().cpu())
            mn = img.min()
            mx = img.max()
            img = ((img - mn) / max(mx - mn, 1e-10)).clip(0, 1)
            img = (img * 255).astype(np.uint8)
            img = cv2.applyColorMap(img, cv2.COLORMAP_INFERNO)
            cv2.imwrite("./images/depth_vis.png", img)
